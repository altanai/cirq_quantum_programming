...
# Quantum state can be transmitted by sending only two classical bits of information.
# This is accomplished by pre-sharing an entangled state between the sender (Alice) and the receiver (Bob).
# This entangled state allows the receiver (Bob) of the two classical bits of information to possess a qubit with the same state
# as the one held by the sender (Alice).

# In the following example output, qubit 0 (the Message) is set to a random state by applying X and Y gates.
# By sending two classical bits of information after qubit 0 (the Message) and qubit 1 (Alice's entangled qubit) are measured,
# the final state of qubit 2 (Bob's entangled qubit) will be identical to the original random state of qubit 0 (the Message).
# This is only possible given that an entangled state is pre-shared between Alice and Bob.
